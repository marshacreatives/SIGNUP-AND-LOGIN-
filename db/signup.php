...
function newUser()
{
		...
		$password=$_POST['pwd'];
		$prepeat=$_POST['pwdr'];
		$sql = "INSERT INTO users (Name, Gender, Contact,Email,Username,Password) VALUES ('$name','$gender','$contact','$email','$username','$password') ";
...
{
	if(!empty($_POST['username']) && !empty($_POST['pwd']) &&!empty($_POST['fname']) &&!empty($_POST['gender']) &&!empty($_POST['email']) && !empty($_POST['contact']))
			checkusername();
}
?>
